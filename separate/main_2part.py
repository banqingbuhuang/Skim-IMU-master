...
import separate.model.rnn_6_2part as nnmodel
import separate.model.rnn_6_1part as nnmodel_1
from utils.opt import Options
from utils import loss_func, utils_utils as utils

IMU_mask_2 = [21, 20, 5, 4, 15, 0, 19, 18, 8, 7]
...
def main(opt):
    ...
    script_name += "0_in{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    print(">>> creating model")
    RNN_pos = nnmodel_1.RNN_pos(input_size=72, output_size=18, hidden_size=256,
                                batch_size=batch_size, num_layers=3, device=device, dropout=0.1)
    RNN_pos_path_len = './checkpoint/test/ckpt_main_1part0_in60_out40_dctn100_best.pth.tar'
    print(">>> loading ckpt len from '{}'".format(RNN_pos_path_len))

    model = nnmodel.GL_RNN_Block(batch_size=batch_size, hidden_size=256, adjs=adjs, dim=3, device=device, dropout=0.1)
    if is_cuda:
        RNN_pos = RNN_pos.to(device)
        ckpt = torch.load(RNN_pos_path_len)
        model = model.to(device)
    else:
        ckpt = torch.load(RNN_pos_path_len, map_location='cpu')

    start_epoch = ckpt['epoch']
    print(">>>  start_epoch", start_epoch)
    RNN_pos.load_state_dict(ckpt['state_dict'])
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        head = np.array(['epoch'])
        # per epoch
        Ir_now, t_l, t_pos, t_bone = train(train_loader, model, RNN_pos, optimizer, cuda=device,
        ...
        # validation
        v_p, v_bone = val(val_loader, model, RNN_pos, cuda=device, loss_weight=loss_weight, all_n=all_n)
        print("val_loss:", v_p)
...
def train(train_loader, model, RNN_pos, optimizer, cuda, loss_weight, lr_now, max_norm, all_n):
    print("进入train")
    # 初始化
    t_l = utils.AccumLoss()
    ...
    for i, (input_ori, input_acc, out_poses, out_joints) in enumerate(train_loader):
        if torch.isnan(input_ori).sum() == 0 and torch.isnan(input_acc).sum() == 0 \
            ...
            if torch.cuda.is_available():
                ...
                out_joints = out_joints[:, :, IMU_mask_2].to(cuda).float()
            # model要改
            posi_6 = RNN_pos(input_ori, input_acc)
            y_out = model(posi_6.transpose(0,1))
...
def val(train_loader, model, RNN_pos, cuda, loss_weight, all_n):
    print("进入val")
