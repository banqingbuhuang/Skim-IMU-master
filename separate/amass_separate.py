...
# 选择 头,根节点，左手腕，左脚踝，右膝盖，右膝盖
# IMU_mask = [3, 7, 4, 8, 0, 1]
...
class amass_rnn(Dataset):
    ...
    def __getitem__(self, item):
        ...
        acc = torch.from_numpy(acc[:, IMU_idx]).float()
        ori = torch.from_numpy(ori[:, IMU_idx]).float()
        ...
        # 数据预处理
        acc, ori = normalize_and_concat(glb_acc=acc, glb_ori=ori)
