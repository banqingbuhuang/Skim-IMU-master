...
class amass_rnn(Dataset):
    def __init__(self, path_to_data, act, input_n, output_n, split=0):
        amass_dir = path_to_data
        tra_val_test = ['train', 'val', 'test']
        subset_split = tra_val_test[split]
        self.data_path = os.path.join(amass_dir, subset_split, act+'.pkl')
        self.seq_len = input_n + output_n  # 采样的长度
        self.input_n = input_n
        self.output_n = output_n
        #
        # 数据的长度
        ori, acc, poses, joints = load_data(self.data_path)
