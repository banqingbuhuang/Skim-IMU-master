...
from utils.utils_math import spatial as S
from utils.utils_math import general as G
import utils.utils_math as M
...
class ParametricModel:
    ...
    def save_unity_motion(pose: torch.Tensor = None, tran: torch.Tensor = None, output_dir='saved_motions/'):
        ...
        if pose is not None:
            ...
            pose = A.rotation_matrix_to_axis_angle(pose).view(pose.shape[0], -1)
    ...
    def bone_vector_to_joint_position(self, bone_vec: torch.Tensor):
        ...
        """
        return S.bone_vector_to_joint_position(bone_vec, self.parent)
    ...
    def joint_position_to_bone_vector(self, joint_pos: torch.Tensor):
        ...
        """
        return S.joint_position_to_bone_vector(joint_pos, self.parent)

    def forward_kinematics_R(self, R_local: torch.Tensor):
        r"""
        :Math_utils:`R_global = FK(R_local)`
        ...
        """
        return S.forward_kinematics_R(R_local, self.parent)

    def inverse_kinematics_R(self, R_global: torch.Tensor):
        r"""
        :Math_utils:`R_local = IK(R_global)`
        ...
        """
        return S.inverse_kinematics_R(R_global, self.parent)

    def forward_kinematics_T(self, T_local: torch.Tensor):
        r"""
        :Math_utils:`T_global = FK(T_local)`
        ...
        """
        return S.forward_kinematics_T(T_local, self.parent)

    def inverse_kinematics_T(self, T_global: torch.Tensor):
        r"""
        :Math_utils:`T_local = IK(T_global)`
        ...
        """
        return S.inverse_kinematics_T(T_global, self.parent)
    ...
    def forward_kinematics(self, pose: torch.Tensor, shape: torch.Tensor = None, tran: torch.Tensor = None,
        ...
        T_local = S.transformation_matrix(pose, self.joint_position_to_bone_vector(j))
        T_global = self.forward_kinematics_T(T_local)
        pose_global, joint_global = S.decode_transformation_matrix(T_global)
        if calc_mesh is False:
            return pose_global, add_tran(joint_global)

        T_global[..., -1:] -= torch.matmul(T_global, G.append_zero(j, dim=-1).unsqueeze(-1))
        ...
        vertex_global = torch.matmul(T_vertex, G.append_one(v, dim=-1).unsqueeze(-1)).squeeze(-1)[..., :3]
    ...
    def view_joint(self, joint_list: list, fps=60, distance_between_subjects=0.8):
        ...
        v_list, f_list = [], []
        f = vc.joints_to_mesh(joint_list[0][0], self.parent, colors=self.parent)[1]
        for i in range(len(joint_list)):
            v = np.stack([vc.joints_to_mesh(frame, self.parent, colors=self.parent)[0] for frame in joint_list[i]])
