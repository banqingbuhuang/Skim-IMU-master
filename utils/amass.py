...
class amass_rnn(Dataset):
    ...
    def __getitem__(self, item):
        ...
        acc = numpy.stack([acc[:, idx] for idx in IMU_idx], axis=1)
        ori = numpy.stack([ori[:, idx] for idx in IMU_idx], axis=1)
        ...
        # 这里是所有 ori ,加上所有 acc
        # amass_acc = acc.reshape(self.seq_len, -1)
        # amass_ori = ori.reshape(self.seq_len, -1)
        #
        # input_ori_acc = np.concatenate((amass_ori, amass_acc), axis=1)
        # input_ori_acc = input_ori_acc[:self.input_n]
        # 这里考虑做每个节点的ori 后面加上acc,
        amass_acc = acc
        amass_ori = ori
...
def normalize_and_concat(glb_acc, glb_ori):
    glb_acc = torch.from_numpy(glb_acc.reshape(-1, len(IMU_idx), 3)).float()
    glb_ori = torch.from_numpy(glb_ori.reshape(-1, len(IMU_idx), 3, 3)).float()
    acc = torch.cat((glb_acc[:, 1:] - glb_acc[:, :1], glb_acc[:, :1]), dim=1).bmm(glb_ori[:, 0]) / acc_scale
    ori = torch.cat((glb_ori[:, :1].transpose(2, 3).matmul(glb_ori[:, 1:]), glb_ori[:, :1]), dim=1)
    # 改成根节点在前的
    return acc, ori
