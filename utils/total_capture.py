...
class amass_rnn(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # acc = numpy.stack([acc[:, idx] for idx in IMU_mask], axis=1)
        # ori = numpy.stack([ori[:, idx] for idx in IMU_mask], axis=1)
        # 如果ori的长度不是4，那就已经转化为rotation_matrix格式了。

        poses = axis_angle_to_rotation_matrix(poses).reshape(self.seq_len, 24, 3, 3)
        # 数据预处理
        """
        先撤一下，看看没有norm的数据怎么样，或许是已经norm过的数据
        """
