r"""
    Spatial Math_utils utils that combine linear and angular calculations for rigid bodies.
...
from utils.utils_math.general import *
import numpy as np
import torch
...
def transformation_matrix_np(R, p):
    ...
    Transformation matrix :Math_utils:`T_{sb} \in SE(3)` of shape [4, 4] can convert points or vectors from b frame
    to s frame: :Math_utils:`x_s = T_{sb}x_b`.
...
def adjoint_transformation_matrix_np(R, p):
    ...
    Adjoint matrix :Math_utils:`[Ad_{T_{sb}}]` of shape [6, 6] can convert spatial twist/wrench/Jacobian between b/s frames.

        :Math_utils:`\mathcal{V}_s = [Ad_{T_{sb}}]\mathcal{V}_b`

        :Math_utils:`\mathcal{F}_b = [Ad_{T_{sb}}]^T\mathcal{F}_s`

        :Math_utils:`J_s = [Ad_{T_{sb}}]J_b`
...
def bone_vector_to_joint_position(bone_vec: torch.Tensor, parent):
    ...
    bone vectors (position difference of child and parent joint)骨骼向量就是骨骼的长度了，不仅反映长度，还反映位置
...
def forward_kinematics(R_local: torch.Tensor, p_local: torch.Tensor, parent):
    r"""
    :Math_utils:`R_global, p_global = FK(R_local, p_local)`
