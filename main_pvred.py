...
def main(opt):
    ...
    script_name += "newNNNN_in{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    ...
    # input_seq,
    model = nnmodel.Encoder_Decoder(input_size=72, hidden_size=1024, num_layer=1, rnn_unit='gru',
...
def train(train_loader, model, optimizer, lr_now, input_n, output_n, device, max_norm, is_cuda, all_n):
    ...
    for i, (input_ori, input_acc, out_poses, out_joints) in enumerate(train_loader):
        if torch.isnan(input_ori).sum() == 0 and torch.isnan(input_acc).sum() == 0 \
            ...
            out_joints = out_joints[:, input_n:].reshape(batch_size, output_n, -1)
            if is_cuda:
            ...
            loss = loss_func.position_loss(outputs_dec, out_joints)
...
def test(train_loader, model, input_n, output_n, device, is_cuda, all_n):
    ...
    # t_bone = np.zeros(len(eval_frame))
    model.eval()
    with torch.no_grad():
        for i, (input_ori, input_acc, out_poses, out_joints) in enumerate(train_loader):
            if torch.isnan(input_ori).sum() == 0 and torch.isnan(input_acc).sum() == 0 \
