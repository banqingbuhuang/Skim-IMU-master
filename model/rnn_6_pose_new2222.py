...
class Predict_imu(nn.Module):
    def __init__(self,
        ...
        self.all_position = RNN_pose(input_frame=input_frame_len,
                                     input_size=mid_2_size + input_size * 3,  # 需要修改
        ...
        self.gcn2 = GC_Block(in_features=output_frame_len, device=device,
                             ...
                             p_dropout=dropout, bias=True, node_n=output_size, dim=9)
...
class RNN_pos(nn.Module):
    ...
    def forward(self, x):
        encoder = x[:self.input_frame]
        decoder = x
        batchsize = encoder.shape[1]
        state = torch.zeros(batchsize, self.hidden_size)
        state = state.to(self.device)
        for i in range(self.input_frame - 1):
            state = self.cell(encoder[i], state)
...
class RNN_pose(nn.Module):
    def __init__(self,
        ...
        # self.cell = nn.GRUCell(input_size, hidden_size)
        # self.fc1 = nn.Linear(hidden_size, input_size)
        self.bisltm = BiLSTM(input_size=input_size,
                             output_size=output_size,
                             hidden_size=hidden_size,
                             num_layers=1,
                             dropout=dropout)
    ...
    def forward(self, x, en):
        encoder_input = torch.cat([x, en, en, en], dim=2)
        encoder = encoder_input[:self.input_frame]
        decoder = encoder_input
        batchsize = encoder.shape[1]
        state = torch.zeros(batchsize, self.hidden_size)
        state = state.to(self.device)
        for i in range(self.input_frame - 1):
