...
class Predict_imu(nn.Module):
    def __init__(self,
        ...
        self.all_pose = RNN_pose(input_frame=input_frame_len,
    ...
    def forward(self, ori, acc):
        ...
        posi_24 = self.all_pose(y, encoder_input)
        posi_24 = self.gcn2(posi_24.transpose(0, 1)).view(b, -1, 24, 3, 3)
...
class GC_Block(nn.Module):
    def __init__(self, in_features, hid_feature, device, p_dropout, adj, bias, node_n, dim=3):
        """
        ...
        self.gc1 = GraphConvolution(in_features, out_features=in_features, device=device,
                                    adj=adj, node_n=node_n, bias=bias, dim=dim)
...
class RNN_pose(nn.Module):
    def __init__(self,
        ...
        # 网络结构
        self.bisltm = BiRNN(input_size=input_size,
                            output_size=output_size,
                            hidden_size=hidden_size,
                            num_layers=2,
                            dropout=dropout)
    ...
    def forward(self, x, en):
        ...
        outputs = self.bisltm(encoder_input)
        return outputs
