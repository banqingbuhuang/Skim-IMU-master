...
class RNN_GRU(nn.Module):
    def __init__(self,
        ...
        self.dropout = dropout
        self.hid_size = 128
        self.cell = nn.GRUCell(input_size, self.hid_size)
    ...
    def forward(self, x):
        ...
        state = torch.zeros(batchsize, self.input_size)  # 输出状态？
        state = state.to(self.device)
        for i in range(input_frame - 1):
            state = self.cell(encoder_in[i], state)
        ...
        for i in range(self.out_frame - input_frame):
            ...
            output = inp + F.dropout(state, self.dropout, training=self.training)
...
class RNN_GRU_2(nn.Module):
    def __init__(self,
                 input_frame, out_frame, input_size, output_size, device, dropout):
        super(RNN_GRU_2, self).__init__()
        self.input_frame = input_frame
        self.input_size = input_size
        self.out_frame = out_frame
        self.device = device
...
class RNN_pos_all(nn.Module):
    def __init__(self,
        ...
        # 网络结构
        mid_size = 48
        self.bilstm_1 = BiRNN(input_size=input_size,
                              output_size=mid_size,
                              hidden_size=hidden_size,
                              num_layers=num_layers,
                              dropout=dropout)
        self.bilstm_2 = BiRNN(input_size=mid_size,
                              output_size=output_size,
                              hidden_size=hidden_size,
                              num_layers=num_layers,
                              dropout=dropout)
    ...
    def forward(self, x, en):
        ...
        outputs = self.bilstm_1(encoder_input)
        outputs = self.bilstm_2(outputs)
        return outputs
