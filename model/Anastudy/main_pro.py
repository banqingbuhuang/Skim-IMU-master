...
import model.Anastudy.rnn_pro as nnmodel
...
def main(opt):
    ...
    script_name += "mmm111_in{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        head = np.array(['epoch'])
        Ir_now, t_l, t_pos, t_pos_0, t_bone = train(train_loader, model, optimizer, loss_weight,
                                                    input_n=input_n,
                                                    output_n=output_n,
                                                    cuda=device, batch_size=batch_size,
        ...
        # validation
        v_p, v_p_0, v_bone = val(val_loader, model, cuda=device, loss_weight=loss_weight,
                                 input_n=input_n,
                                 output_n=output_n,
        ...
        # test
        test_position = test(train_loader=test_dip_loader, cuda=device, model=model, input_n=input_n, all_n=all_n
                             )
        ...
        for act in actions:
            test_position = test(train_loader=test_loader[act], cuda=device, model=model, input_n=input_n, all_n=all_n
                                 )
            position = act + 'po_'
...
def train(train_loader, model, optimizer, loss_weight, input_n, output_n, cuda, batch_size, lr_now, max_norm, all_n):
    print("进入train")
    # 初始化
    t_l = utils.AccumLoss()
...
def val(train_loader, model, cuda, input_n, output_n, loss_weight, batch_size, all_n):
    print("进入val")
...
def test(train_loader, model, cuda, input_n, all_n):
    print("进入test")
    N = 0
