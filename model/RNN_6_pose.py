...
class Predict_imu(nn.Module):
    def __init__(self,
        ...
        self.gcn1 = GC_Block(in_features=output_frame_len, device=device,
                             ...
                             p_dropout=dropout, bias=True, node_n=mid_size, dim=dim)
        self.progressive = Progressive(frame_num=input_frame_len,
                                       ...
                                       dropout=dropout,
                                       dim=dim)
        self.all_position = RNN_pose(input_frame=input_frame_len,
                                 input_size=mid_2_size + input_size * 2,  # 需要修改
                                 output_size=output_size,
                                 hidden_size=512,
                                 num_layers=2,
                                 device=device,
                                 dropout=dropout)
        self.gcn2 = GC_Block(in_features=output_frame_len, device=device,
                             hid_feature=output_frame_len, adj=adjs[-1],
                             p_dropout=dropout, bias=True, node_n=output_size, dim=dim)
    ...
    def forward(self, ori, acc):
        ...
        pose_24 = self.all_position(y, encoder_input)
        pose_24 = self.gcn2(pose_24.transpose(0, 1)).view(b, f, 24, -1)
...
class Progressive(nn.Module):
    def __init__(self, frame_num,
                 input_size, adjs, device, dropout, dim):
        super(Progressive, self).__init__()
        mid_size = [7, 8, 9, 10]
        self.up_7 = Up(frame_num=frame_num,
                       input_size=input_size,
                       output_size=mid_size[0] * dim,
                       adj=adjs[2],
                       device=device,
                       dropout=dropout)
        self.up_8 = Up(frame_num=frame_num,
                       input_size=mid_size[0] * dim,
                       output_size=mid_size[1] * dim,
                       adj=adjs[4],
                       device=device,
                       dropout=dropout)
        self.up_9 = Up(frame_num=frame_num,
                       input_size=mid_size[1] * dim,
                       output_size=mid_size[2] * dim,
                       adj=adjs[6],
                       device=device,
                       dropout=dropout)
        self.up_10 = Up(frame_num=frame_num,
                        input_size=mid_size[2] * dim,
                        output_size=mid_size[3] * dim,
                        adj=adjs[8],
                        device=device,
                        dropout=dropout)
...
class Up(nn.Module):
    def __init__(self, frame_num,
        ...
        self.up = nn.Linear(in_features=input_size, out_features=9)
...
class GraphConvolution(nn.Module):
    ...
    def __init__(self, in_features, out_features, adj, device, bias=True, node_n=48, dim=9):
...
class GC_Block(nn.Module):
    def __init__(self, in_features, hid_feature, device, p_dropout, adj, bias, node_n, dim):
        """
        ...
        self.gc1 = GraphConvolution(in_features, out_features=in_features,
                                    device=device, adj=adj, node_n=node_n, bias=bias, dim=dim)
...
class RNN_pose(nn.Module):
    def __init__(self,
        ...
        # 网络结构
        # self.cell = nn.LSTM(input_size, input_size,num_layers=2)
        self.bilstm = BiLSTM(input_size=input_size,
                             output_size=output_size,
                             hidden_size=hidden_size,
                             num_layers=num_layers,
    ...
    def forward(self, x, en, hid=None):
        encoder_input = torch.cat([x, en, en], dim=2)
        # encoder_input, hid = self.cell(encoder_input, hid)
