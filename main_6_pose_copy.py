...
def main(opt):
    ...
    script_name += "pose_dip_in{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    ...
    # data_dip_dir
    train_dataset = amass(path_to_data=opt.data_dip_total_short_dir, input_n=input_n, output_n=output_n,
                          split=0)
    val_dataset = amass(path_to_data=opt.data_dip_total_short_dir, input_n=input_n, output_n=output_n,
                        split=1)
    test_dataset = amass(path_to_data=opt.data_dip_total_short_dir, input_n=input_n, output_n=output_n,
                         split=2)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        # test
        test_all = test(train_loader=test_loader, model=model, cuda=device,
                        ignore=ignore,
