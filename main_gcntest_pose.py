...
def main(opt):
    ...
    script_name += "nnnnn{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    # 返回main_in10_out25_dctn35
    print(">>> creating model")
    # 将adjs放入cuda
    # in_features, adjs, node_n, dim, p_dropout,
    # 维度为dim dim=3表示三维位置，dim=9表示rotation matrix
    model = nnmodel.GCN_pose(input_feature=all_n, hidden_feature=opt.linear_size, p_dropout=0.5,
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        test_full = test(test_dip_loader, model, input_n=input_n, output_n=output_n, ignore=ignore, device=device,
                         is_cuda=is_cuda,
                         all_n=all_n)
        ...
        for act in actions:
            test_full = test(test_loader[act], model, input_n=input_n, output_n=output_n, ignore=ignore, device=device,
                             is_cuda=is_cuda,
                             all_n=all_n)
            sm, am, pm, me = 'sip_m', 'angle_m', 'pos_m', 'Mesh_m'
            ret_log = np.append(ret_log, test_full)
...
def get_idct(y_out, out_joints, device):
    ...
    # 32,72,50
    pred_3d = outputs_p3d.contiguous().view(-1, 9).contiguous()
    targ_3d = out_joints.contiguous().view(-1, 9).contiguous()
...
def train(train_loader, model, optimizer, lr_now, input_n, output_n, device, max_norm, is_cuda, all_n):
    ...
    for i, (input_ori, input_acc, out_poses, out_joints) in enumerate(train_loader):
        if torch.isnan(input_ori).sum() == 0 and torch.isnan(input_acc).sum() == 0 \
            ...
            # 计算loss
            pred_3d, targ_3d = get_idct(y_out=y_out, out_joints=out_poses, device=device)
        ...
        for i, (input_ori, input_acc, out_poses, out_joints) in enumerate(train_loader):
            if torch.isnan(input_ori).sum() == 0 and torch.isnan(input_acc).sum() == 0 \
                ...
                if is_cuda:
                    ...
                    out_poses = out_poses.to(device).float()
                y_out = model(model_input)  # 32,72,50
                batch, frame, node, dim = out_poses.data.shape
                ...
                outputs_p3d = outputs_p3d.reshape(frame, batch, node, dim).contiguous().permute(1, 0, 2, 3).contiguous()
                # 32,72,50
                # pred_3d = outputs_p3d[:, input_n:, :].contiguous()
                # targ_3d = out_joints[:, input_n:, :].contiguous()

                for k in np.arange(0, len(eval_frame)):  # 6
                    j = eval_frame[k]
                    test_out, test_poses = outputs_p3d[:, j, :, :], out_poses[:, j, :, :]
...
def main_total(opt):
    ...
    script_name += "0total_in{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    # 返回main_in10_out25_dctn35
    print(">>> creating model")
    # 将adjs放入cuda
    # in_features, adjs, node_n, dim, p_dropout,
    # 维度为dim dim=3表示三维位置，dim=9表示rotation matrix
    model = nnmodel.GCN(input_feature=all_n, hidden_feature=opt.linear_size, p_dropout=0.5,
                        num_stage=opt.num_stage, node_n=216)
    ...
    # data_amass_dir
    # data_benji_dir
    train_dataset = total(path_to_data=opt.data_total_dir, input_n=input_n, output_n=output_n,
                          split=0)
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,  # 在每个epoch开始的时候，对数据进行重新排序
        num_workers=6,  # 原来写的是opt.job=10，现在因为虚拟内存不够，更改为0。
        # 这个参数决定了有几个进程来处理data loading。0意味着所有的数据都会被load进主进程。（默认为0）
        pin_memory=False)
    val_loader = DataLoader(
        dataset=val_dataset,
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        ret_log = np.append(ret_log, [v_p, v_bone])
        head = np.append(head, ['v_p', 'v_bone'])
        test_position_temp = np.array([])
        test_3d_head = np.array([])
        test_bone_temp = np.array([])
        test_bone_head = np.array([])
        for act in actions:
            test_position, test_bone = test(test_loader[act], model, input_n=input_n, output_n=output_n, ignore=ignore,
                                            device=device,
                                            is_cuda=is_cuda,
                                            all_n=all_n)
            position = act + 'po_'
            ...
            test_position_temp = np.append(test_position_temp, test_position)
            test_bone_temp = np.append(test_bone_temp, test_bone)
            test_3d_head = np.append(test_3d_head,
                                     [position + '5', position + '11', position + '17', position + '23',
                                      position + '29', position + '35', position + '36',
                                      position + '38', position + '41', position + '45', position + '47',
                                      position + '50',
                                      position + '53', position + '56', position + '59'
                                      ])
            test_bone_head = np.append(test_bone_head,
                                       [bone + '5', bone + '11', bone + '17', bone + '23', bone + '29', bone + '35',
                                        position + '36',
                                        bone + '38', bone + '41', bone + '44', bone + '47', bone + '50',
                                        bone + '53', bone + '56', bone + '59'
                                        ])
        ret_log = np.append(ret_log, [test_position_temp, test_bone_temp])  # 内容
        head = np.append(head, [test_3d_head, test_bone_head])  # 表头
        if not np.isnan(v_p):  # 判断空值 只有数组数值运算时可使用如果v_e不是空值
            is_best = v_p < err_best  # err_best=10000
            err_best = min(v_p, err_best)
        else:
...
def main_total_pose(opt):
    ...
    script_name += "2_total_pose_in{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    # 返回main_in10_out25_dctn35
    print(">>> creating model")
    # 将adjs放入cuda
    # in_features, adjs, node_n, dim, p_dropout,
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for act in actions:
            test_full = test(test_loader[act], model, input_n=input_n, output_n=output_n, ignore=ignore,
                             device=device,
                             is_cuda=is_cuda,
                             all_n=all_n)
            sm, am, pm, me = act + 'sip_m', act + 'angle_m', act + 'pos_m', act + 'Mesh_m'
            ret_log = np.append(ret_log, test_full)
            head = np.append(head,
                             [sm + '5', am + '5', pm + '5', me + '5',
