...
def main(opt):
    ...
    data_path = "/data/xt/dataset/DIP-total_short"
    train_dataset = amass(path_to_data=data_path, input_n=input_n, output_n=output_n,
                          split=0)
    val_dataset = amass(path_to_data=data_path, input_n=input_n, output_n=output_n,
                        split=1)
    test_dip_dataset = amass(path_to_data=data_path, input_n=input_n, output_n=output_n,
                             split=2)
    train_loader = DataLoader(
        dataset=train_dataset,
        batch_size=32,  # batch 32
        shuffle=True,  # 在每个epoch开始的时候，对数据进行重新排序
        num_workers=10,  # 原来写的是opt.job=10，现在因为虚拟内存不够，更改为0。
    ...
    for i, (input_ori, input_acc, out_poses, out_joints) in enumerate(train_loader):
        if torch.isnan(input_ori).sum() == 0 and torch.isnan(input_acc).sum() == 0 \
            ...
            for k in np.arange(0, len(eval_frame)):
                ...
                # print(torch.mean(torch.norm(test_out - test_joints, 2, 2)))
            N += batch
    print(t_posi / N)
    return t_posi / N
...
def eval(opt):
    ...
    # new_2:去掉两个GCN块
    # new_3:只看后半段预测部分的
    script_name += "evalAA111_in{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    # 返回main_in10_out25_dctn35
    print(">>> creating model")
    # 将adjs放入cuda
    # in_features, adjs, node_n, dim, p_dropout,
    ...
    if is_cuda:
        model = model.to(device)
    model_gcn_path = "/home/xt/Skim-IMU-master/checkpoint/test/ckpt_main_dip_total_shortAA111_in36_out24_dctn60_best.pth.tar"
    ...
    model.load_state_dict(ckpt_2['state_dict'])
