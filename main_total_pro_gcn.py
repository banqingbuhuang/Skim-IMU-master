...
def main(opt):
    ...
    script_name += "mmm222_in{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        # test
        test_position = test(train_loader=test_dip_loader, cuda=device, model=model,
                             )
        ...
        for act in actions:
            test_position = test(train_loader=test_loader[act], cuda=device, model=model,
                                 )
            position = act + 'po_'
...
def test(train_loader, model, cuda):
    print("进入test")
    N = 0
