...
import model.TCN_GCN as nnmodel
...
def main(opt):
    ...
    # new_2:去掉两个GCN块
    # new_3:只看后半段预测部分的
    script_name += "MMMM_in{:d}_out{:d}_dctn{:d}".format(opt.input_n, opt.output_n, opt.all_n)
    # 返回main_in10_out25_dctn35
    print(">>> creating model")
    # 将adjs放入cuda
    adj = torch.as_tensor(adjs[-1])
    # in_features, adjs, node_n, dim, p_dropout,
    # 维度为dim dim=3表示三维位置，dim=9表示rotation matrix
    model = nnmodel.Deep_Priors(input_length=all_n, node_n=24, Adj=adj,device=device)
    if is_cuda:
    ...
    val_dataset = amass(path_to_data=opt.data_dip_total_dir, input_n=input_n, output_n=output_n,
                        split=1)
    test_dip_dataset = amass(path_to_data=opt.data_dip_total_dir, input_n=input_n, output_n=output_n,
                             split=2)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        test_position = test(train_loader=test_dip_loader, device=device, model=model,
                             ignore=ignore,output_n=output_n,
        ...
        for act in actions:
            test_position = test(train_loader=test_loader[act], device=device, model=model,
                                 ignore=ignore,
                                 input_n=input_n, output_n=output_n,all_n=all_n)
            position = act + 'po_'
...
def test(train_loader, model, device, ignore, input_n, output_n,all_n):
    print("进入test")
    N = 0
    # 100,200,300,400,500,1000
    # eval_frame = [2, 5, 8, 11, 14, 29]
    # eval_frame = [2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, 38, 41, 49]
    eval_frame = [5, 17, 35, 36, 41, 47, 53, 59]
